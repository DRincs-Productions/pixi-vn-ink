...
export function getLabelByStandardDivert(divertName: string, labelKey: string): string | undefined {
    ...
    if (
        ...
        return (
            ...
            CHOISE_LABEL_KEY_SEPARATOR +
            endOfLabel.replaceAll(".", CHOISE_LABEL_KEY_SEPARATOR)
        );
    }
    return (
        divertName.replaceAll(".", CHOISE_LABEL_KEY_SEPARATOR) ||
        getLabelByStandardDivertInternal(labelKey, counter).replaceAll(
            ".",
            CHOISE_LABEL_KEY_SEPARATOR,
        )
    );
