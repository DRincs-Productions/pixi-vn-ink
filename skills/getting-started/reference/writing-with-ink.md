> Verbatim copy of Inkle's official ["Writing with ink"](https://github.com/inkle/ink/blob/master/Documentation/WritingWithInk.md) guide, bundled as reference material for the `pixi-vn-ink` skill. It documents **native ink** — the language as it runs in Inky/inkjs with no game engine attached. It says nothing about Pixi'VN.
>
> Read `../SKILL.md` first: it covers what to use from this reference, what Pixi'VN ink ignores or changes (knots vs. labels, no Threads, `LIST` entries need their qualified name, `TURNS()`/`TURNS_SINCE()`/`SEED_RANDOM()`/most `LIST_*()` functions not implemented yet, etc.), and everything Pixi'VN adds on top (hashtag commands for canvas/sound/assets/input, character dialogue, text replacement, i18n). Come back to this file only when a native-ink syntax detail needs the full spec or a longer worked example (Tower of Hanoi, the crime-scene mystery, turning numbers into words, etc.).
...
> **Pixi'VN ink:** the opposite is true. Content outside any `=== knot ===` is **ignored** — except a top-level `VAR`/`CONST` declaration, which is still handled. A top-level `-> start` divert like the one below does nothing in Pixi'VN; a knot only ever runs when JS/TS explicitly starts it with `narration.call("start", {})` / `narration.jump(...)` (typically after `importInkText(...)`). See [SKILL.md § Syntax Pixi'VN ignores entirely](../SKILL.md#syntax-pixivn-ignores-entirely).
...
> **Pixi'VN ink:** `INCLUDE` is parsed but **ignored** — it has no effect. To combine multiple ink sources, pass them all to `importInkText([sourceA, sourceB, ...])` instead; every knot registers into the same global label registry regardless of which string it came from, same as native ink's file-independent namespacing. See [SKILL.md § Syntax Pixi'VN ignores entirely](../SKILL.md#syntax-pixivn-ignores-entirely).
...
> **Pixi'VN ink:** none of `CHOICE_COUNT()`, `TURNS()`, `TURNS_SINCE()`, or `SEED_RANDOM()` — the whole
> of this section — is implemented by the Pixi'VN ink runtime yet. They parse without error but won't
> behave as described below. See [SKILL.md § Not yet supported in Pixi'VN
> ink](../SKILL.md#not-yet-supported-in-pixi-vn-ink) for the current status before relying on any of
> them.
...
> **Pixi'VN ink:** everything in this section is native-ink-only. `=== function ... === / ~ return ...`
> definitions are parsed but **silently ignored** at runtime — calling one does nothing and returns
> nothing. Define equivalent logic in JavaScript/TypeScript instead: **prefer a hashtag command** — see
> the `pixi-vn-ink-hashtag-commands` skill's [Why not an ink
> `function`?](../../hashtag-commands/SKILL.md#why-not-an-ink-function) — over exposing a plain function
> through `StepLabelProps`. See also [SKILL.md § Functions: define in JS/TS (prefer a hashtag
> command), not ink](../SKILL.md#functions-define-in-jsts-prefer-a-hashtag-command-not-ink).
...
> **Pixi'VN ink:** Threads (as described in this whole section — forking the story and merging choice
> lists from multiple sources) are **not supported**. The `<-` symbol is instead repurposed to mean the
> same thing as a tunnel, `-> knot ->` — it **calls** the target knot and returns, rather than forking
> and later collapsing multiple branches. Don't expect combined choice lists, or the "shared but not
> forked" global-variable semantics described below, when porting a script that used `<-`. See
> [SKILL.md § Threads work differently in Pixi'VN ink](../SKILL.md#threads-work-differently-in-pixi-vn-ink).
...
> **Pixi'VN ink:** every `LIST` entry is stored under its **qualified name**, `listName.entryName`
> (e.g. `kettleState.cold`), and — unlike native ink — Pixi'VN does **not** treat the bare entry name
> (`cold`) as equivalent. Writing just `cold` reads/writes an unrelated plain storage key called
> `"cold"`, not the list member, with no warning. Always use the qualified form throughout this whole
> Part 5, even in the examples below that use the bare name. See [SKILL.md § LIST entries need their
> qualified name](../SKILL.md#list-entries-need-their-qualified-name).
...
> **Pixi'VN ink:** `LIST_COUNT()`, `LIST_MIN()`, `LIST_MAX()`, and `LIST_RANDOM()` — plus `LIST_ALL()`
> and `LIST_VALUE()` further below — are **not implemented yet** by the Pixi'VN ink runtime. `has`/`?`,
> `hasnt`/`!?`, `==`/`!=`, and the emptiness test (`{ list: ... }`) all work as described; the numeric
> comparisons (`<`, `>`, `<=`, `>=`) and `++`/`--` also work. See [SKILL.md § Not yet supported in
> Pixi'VN ink](../SKILL.md#not-yet-supported-in-pixi-vn-ink).
...
> **Pixi'VN ink:** the comparisons in "Comparing lists" below (`>`, `<`, `>=`, `<=`) and the `^`
> intersection operator work as described. `LIST_INVERT()` and `LIST_RANGE()`, however, are **not
> implemented yet** — see [SKILL.md § Not yet supported in Pixi'VN
> ink](../SKILL.md#not-yet-supported-in-pixi-vn-ink).
